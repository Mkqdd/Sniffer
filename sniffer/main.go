...
func main() {
	...
	if cfg.SSLBlacklistDir != "" {
		// 将相对路径解析为“配置文件所在目录”的相对路径，避免从其他 cwd 启动时找不到
	...
	// Start Maltrail UDP receiver when enabled
	if cfg.MaltrailEnabled && cfg.MaltrailUDPListen != "" {
		maltrailRecv := maltrail.NewReceiver(cfg, st)
		go func() {
			if err := maltrailRecv.Run(ctx); err != nil && ctx.Err() == nil {
				log.Printf("Maltrail receiver stopped: %v", err)
			}
		}()
	}
